...
const Score = () => {
  ...
  const fetchData = async () => {
    const response = await axiosJWT.get(`${process.env.NEXT_PUBLIC_BASE_URL}/mardiyuana-student/score-course?courseId=${courseId}&id=${enrollmentId}&type=${type}`, {
          ...
          {scoreCoursesData?.map((score: ScoreCourseList, idx) => {
            return (
                ...
                >
                  ...
                  <p className="text-sm font-semibold">{score.type ==="ASSIGNMENT" ? "due" : "started at"}, {moment(score.scoreDue).format("LL")}</p>
                </div>
            )
          })}
          </section>
